...
class TriplaneViTNetwork(nn.Module):
...
if __name__ == '__main__':
    print("Debugging ViT for Triplane Encoder ...")
    # Create an instance of the network
    model = TriplaneViTNetwork().cuda()

    # Create a dummy input tensor with dimensions Nx256x64x64
    dummy_input = torch.randn(4, 192, 256, 256).cuda()
    dummy_F = torch.randn(4, 96, 256, 256).cuda()

    # Test the forward pass
    output = model(dummy_input, dummy_F)
    print(output.shape)
    print("Finished ViT for Triplane Encoder!")
