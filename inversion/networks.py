...
class LatentDecoder_perStyle(nn.Module):
    def __init__(self, emap_dim):
        super(LatentDecoder_perStyle, self).__init__()
        self.latent_dim = 512
        self.emap_dim = emap_dim

        self.fc1 = nn.Linear(self.latent_dim+self.emap_dim, 512)
...
class LatentDecoder(nn.Module):
    def __init__(self, emap_dim):
        super().__init__()
        self.decoder_perStyle = nn.ModuleList()
        for i in range(14):
            x = LatentDecoder_perStyle(emap_dim=emap_dim)
            self.decoder_perStyle.append(x)

    def forward(self, lat, env):
        out = []
        B = lat.shape[0]
        # env = env.view(B, -1)
        env = env.reshape(B, -1)
        for i in range(14):
            x = torch.cat([lat[:,i,:], env], 1)
            out.append(self.decoder_perStyle[i](x))
...
class ReflectanceNetwork(torch.nn.Module):
    def __init__(self,
        ...
        super().__init__()
        # Relighting Specific
        self.latent_decoder = LatentDecoder(emap_dim)

    def forward(self, w, emap):
        return self.latent_decoder(w, emap)
...
class Net(nn.Module):
    ...
    def set_reflectance_network(self, device, opts):
        if opts.emap_mode == 'emaps_ds':
            emap_dim = 200*3
        elif opts.emap_mode == 'emaps_sampled':
            emap_dim = 150*3
        else:
             raise NotImplementedError

        R_new = ReflectanceNetwork(emap_dim=emap_dim).eval().requires_grad_(False).to(device)
        ...
        if opts.R_ckpt is not None:
            ...
            R_optim = network['optimizer']
            try:
                misc.copy_params_and_buffers(R, R_new, require_all=True)
            except AssertionError:
                misc.copy_params_and_buffers(R.reflectance_network, R_new, require_all=True)
            R = R_new
        else:
            start_iter = 0
            R = R_new
            R_optim = torch.optim.Adam(
                [
                    {"params": R.latent_decoder.parameters(), "lr": opts.lr, "betas": (0.0, 0.99)},
    ...
    def forward(self, x, c, x_512, use_AFA=True, fix_density=False):
        B = x.shape[0]
        ...
        w_recon_dict = self.decoder.synthesis(ws=rec_ws, c=c, triplane=triplane, triplane_x=triplane_x, train_forward_2=True, noise_mode='const')

        ## AFA
        rec_img_w = w_recon_dict['image']
        if use_AFA:
            feature_map_adain, gamma, beta = self.afa(x_512, rec_img_w, triplane_x)
            full_recon_dict = self.decoder.synthesis(ws=rec_ws, c=c, triplane=triplane, triplane_x=feature_map_adain, train_forward_2=True, noise_mode='const')
        else:
            # WARNING: how is without AFA and occlusion mix going to work?
            full_recon_dict = self.decoder.synthesis(ws=rec_ws, c=c, triplane=triplane, triplane_x=triplane_x, train_forward_2=True, noise_mode='const')
            feature_map_adain = None

        ## Occlusion-Aware mix Fusion
        mix_triplane, mask = self.mix_fusion(B, full_recon_dict, w_recon_dict)

        full_recon_dict['mix_triplane'] = mix_triplane
        full_recon_dict['triplane_x'] = triplane_x
        full_recon_dict['feature_map_adain'] = feature_map_adain
        full_recon_dict['rec_ws'] = rec_ws

        return full_recon_dict, w_recon_dict

    def forward_relight(self, ws_rec, relit_ws, x_512, c, triplane_x_rec=None, feature_map_adain_rec=None):
        """
        Forward rendering without reusing density samples from reconstruction
        Args:
            ws_rec: tensor
            relit_ws: tensor
            x_512: image
            c: camera pose
            triplane_x_rec: reconstruction triplane
            feature_map_adain_rec: StyleGAN feature block

        Returns:
            Final Reconstructed Image along with Mix-Triplane
        ...
        # Relight
        triplane_relit, triplane_x_relit = self.decoder.synthesis(ws=relit_ws, c=c, train_forward_1=True, noise_mode='const')
        # Get W-Triplane for Occlusion Mix-Fusion
        rec_img_dict_w_relit = self.decoder.synthesis(ws=relit_ws, c=c, triplane=triplane_relit, triplane_x=triplane_x_relit,
                                                      forward_relight=True, noise_mode='const')

        ## F space
        feature_map_adain = feature_map_adain_rec + (triplane_x_relit - triplane_x_rec)
        # feature_map_adain = triplane_x_relit

        img_dict = self.decoder.synthesis(ws=ws_rec, c=c, triplane=triplane_relit, triplane_x=feature_map_adain,
                                          forward_relight=True, fix_density=False, noise_mode='const')


        ## Occlusion-Aware mix Fusion
        mix_triplane, mask = self.mix_fusion(B, img_dict, rec_img_dict_w_relit)

        img_edit_dict = self.decoder.synthesis(ws=relit_ws, c=c, triplane=mix_triplane, forward_triplane=True, noise_mode='const')
        img_edit_dict['mix_triplane'] = mix_triplane

        return img_edit_dict, None

    def forward_relight_fix_density(self, ws_rec, relit_ws, x_512, c, triplane_x_rec=None, feature_map_adain_rec=None, rec_triplane_samples=None, fix_density=False):
        B = relit_ws.shape[0]

        # Relight
        triplane_relit, triplane_x_relit = self.decoder.synthesis(ws=relit_ws, c=c, train_forward_1=True, noise_mode='const')
        rec_img_dict_w_relit = self.decoder.synthesis(ws=relit_ws, c=c, triplane=triplane_relit, triplane_x=triplane_x_relit,
                                                      triplane_samples=rec_triplane_samples, forward_relight=True, fix_density=fix_density,
                                                      noise_mode='const')

        img_relit_dict_w = rec_img_dict_w_relit['image']

        ## F space
        feature_map_adain = feature_map_adain_rec + (triplane_x_relit - triplane_x_rec)
        # feature_map_adain = triplane_x_relit

        # WARNING: changed ws_rec to relit_ws
        img_dict = self.decoder.synthesis(ws=relit_ws, c=c, triplane=triplane_relit, triplane_x=feature_map_adain,
                                          triplane_samples=rec_triplane_samples, forward_relight=True,
                                          noise_mode='const')

        ## Occlusion-Aware mix Fusion
        mix_triplane, mask = self.mix_fusion(B, img_dict, rec_img_dict_w_relit)

        img_edit_dict = self.decoder.synthesis(ws=relit_ws, c=c, triplane=mix_triplane, forward_triplane=True,
                                               triplane_samples=rec_triplane_samples, fix_density=fix_density, noise_mode='const')

        img_edit_dict['mix_triplane'] = mix_triplane

        return img_edit_dict, img_relit_dict_w
