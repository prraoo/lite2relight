...
class ImageFolderDataset(Dataset):
    ...
    def _load_raw_image(self, raw_idx, resolution=None):
        ...
        if self.use_512:
            image = cv2.imread(os.path.join(self._path, 'crop', fname))[:,:,::-1]
            # Make copies
            image512 = np.array(image).transpose(2, 0, 1)
            image = cv2.resize(image, (resolution, resolution)).transpose(2, 0, 1)
            # Load Masks
            base_masks_path  = self._path
            mask_path = os.path.join(base_masks_path,'mask_rmbg2', f'{cam}_{id}_EMAP-999.png')
