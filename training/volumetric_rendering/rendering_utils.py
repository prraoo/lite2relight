...
def create_cam2world_matrix(forward_vector, origin, device=None):
    ...
    # EG3D-Fix - Assumes y-axis is up and that there is no camera roll.
...
class RealGaussianCameraPoseSampler:
    """
    Samples roll, pitch and yaw from a Gaussian distribution and returns a camera pose.
    Camera is specified as looking at the origin.
    ...
    @staticmethod
    def sample_params(mean, stddev, batch_size, device='cpu'):
        return torch.randn((batch_size, 1), device=device) * stddev + mean

    @staticmethod
    def FOV_to_intrinsics(fov_degrees_mean, fov_degrees_stddev, cx_cy_mean, cx_cy_stddev, batch_size, device='cpu'):
        """
        Creates a 3x3 camera intrinsics matrix from the camera field of view, specified in degrees.
        Note the intrinsics are returned as normalized by image size, rather than in pixel units.
        Assumes principal point is at image center.
        ...
        fov_degrees = RealGaussianCameraPoseSampler.sample_params(mean=fov_degrees_mean, stddev=fov_degrees_stddev, batch_size=batch_size)
        focal_length = (1 / (torch.tan(fov_degrees * 3.14159 / 360) * 1.414)).squeeze()
        cx_cy = RealGaussianCameraPoseSampler.sample_params(mean=cx_cy_mean, stddev=cx_cy_stddev, batch_size=batch_size).squeeze()

        intrinsics = torch.zeros((batch_size, 3, 3), device=device)
        intrinsics[:, 0, 0] = focal_length
        intrinsics[:, 1, 1] = focal_length
        intrinsics[:, 0, 2] = cx_cy
        intrinsics[:, 1, 2] = cx_cy

        return intrinsics

    @staticmethod
    def roll_rotation_matrix(roll, batch_size, device='cpu'):
        # Generate a rotation matrix for roll
        cos_roll = torch.cos(roll).squeeze()
        sin_roll = torch.sin(roll).squeeze()

        rotation_matrix = torch.zeros((batch_size, 3, 3), device=device)
        rotation_matrix[:, 0, 0] = 1
        rotation_matrix[:, 1, 1] = cos_roll
        rotation_matrix[:, 1, 2] = -sin_roll
        rotation_matrix[:, 2, 1] = sin_roll
        rotation_matrix[:, 2, 2] = cos_roll

        return rotation_matrix

    @staticmethod
    def sample(horizontal_mean, vertical_mean, horizontal_stddev=0, vertical_stddev=0, roll_mean=0, roll_stddev=0, radius_mean=0, radius_stddev=0,  batch_size=1, device='cpu'):

        radius = torch.randn((batch_size, 1), device=device) * radius_stddev + radius_mean  # Sample roll angle
        h = torch.randn((batch_size, 1), device=device) * horizontal_stddev + horizontal_mean
        v = torch.randn((batch_size, 1), device=device) * vertical_stddev + vertical_mean
        r = torch.randn((batch_size, 1), device=device) * roll_stddev + roll_mean  # Sample roll angle
        v = torch.clamp(v, 1e-5, math.pi - 1e-5)
        ...
        roll_rot_matrix = RealGaussianCameraPoseSampler.roll_rotation_matrix(r, batch_size, device)
        camera_origins = torch.bmm(roll_rot_matrix, camera_origins.unsqueeze(-1)).squeeze(-1)
...
if __name__ == '__main__':
    ...
    RealGaussianCameraPoseSampler.sample(math.pi / 2, math.pi / 2, roll_mean=0, roll_stddev=math.pi / 4, radius=1)
